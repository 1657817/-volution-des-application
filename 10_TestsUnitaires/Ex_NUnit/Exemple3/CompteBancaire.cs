...
namespace Exemple3
{
    public class CompteBancaire
    {
        ...
        public const string MsgMontantDebitDepasseSolde = "Le montant de débit excède le solde actuel";
        public const string MsgMontantDebitNegatif = "Montant de débit est négatif";
        ...
        public void Debit(double montant)
        {
            if (montant > m_solde)
            {
                throw new ArgumentOutOfRangeException("montant", montant, MsgMontantDebitDepasseSolde);
            }

            if (montant < 0)
            {
                throw new ArgumentOutOfRangeException("amount", montant, MsgMontantDebitNegatif);
            }
